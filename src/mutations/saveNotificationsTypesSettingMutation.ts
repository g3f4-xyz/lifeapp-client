...
import {
  ...
  saveNotificationsTypesSettingMutationInput, saveNotificationsTypesSettingMutationResponse,
...
export default (
  { types }: saveNotificationsTypesSettingMutationInput,
): Promise<saveNotificationsTypesSettingMutationResponse> => new Promise((onCompleted, onError): void => {
  const variables = { input: { types } };

  commitMutation<saveNotificationsTypesSettingMutation>(
    environment,
    {
      mutation,
      variables,
