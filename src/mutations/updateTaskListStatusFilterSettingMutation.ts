...
): Promise<updateTaskListStatusFilterSettingMutationResponse> => new Promise((onCompleted, onError): void => {
  const variables = { input: { status } };

  commitMutation<updateTaskListStatusFilterSettingMutation>(
    environment,
    {
      mutation,
      variables,
